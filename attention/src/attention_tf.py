...
def Attention(Q, K, V, nb_head, size_per_head, Q_len=None, V_len=None):
    ...
    K = tf.reshape(K, (-1, tf.shape(Q)[1], nb_head, size_per_head))
    V = tf.reshape(V, (-1, tf.shape(Q)[1], nb_head, size_per_head))
    ...
    O = tf.reshape(O, (-1, tf.shape(0)[-1], nb_head * size_per_head))
    O = Mask(O, Q_len, 'mul')
    return O
